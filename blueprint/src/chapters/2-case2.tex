...
\begin{proof}
    ...
    By contradiction we assume $\lambda^2 \divides a + \eta b$, which implies that
    ...
    $\lambda \divides b$, which contradicts \Cref{def:Solution} forcing us to conclude that
    $\lambda^2 \notdivides a + \eta b$.
\end{proof}
...
\begin{proof}
    \leanok
    \uses{lmm:multiplicity_lambda_c_finite}
    By contradiction we assume $\lambda \divides w$, which implies
