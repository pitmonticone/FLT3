...
\begin{proof}
    ...
    % By \Cref{thm:fermatLastTheoremThree_case_1} we can assume $3 \divides a \cdot b \cdot c$
...
\begin{proof}
    ...
    It directly follows from \ref{lmm:lambda_not_unit}.
\end{proof}
...
\begin{proof}
    ...
    \ref{lmm:lambda_pow_four_dvd_cube_sub_one_or_add_one_of_lambda_not_dvd}. \\
    Since $\lambda \notdivides b$, then
    ...
    \begin{itemize}
        ...
        which is absurd by \ref{lmm:lambda_not_dvd_two}.
        \item Case $\lambda^4 \divides a^3 + 1 \land \lambda^4 \divides b^3 + 1$.
...
\begin{proof}
    ...
    Apply \ref{lmm:a_cube_b_cube_same_congr} and then compute each case.
\end{proof}
...
\begin{proof}
    ...
    lambda_prime}
    Trivial applying \ref{lmm:lambda_pow_four_dvd_c_cube}.
\end{proof}
...
\begin{proof}
    ...
    It directly follows from \ref{lmm:lambda_pow_two_dvd_c}.
\end{proof}
...
\begin{proof}
    ...
    It directly follows from \ref{lmm:Solution1_two_le_multiplicity}.
\end{proof}
...
\begin{proof}
    ...
    Straightforward calculation using \ref{lmm:toInteger_cube_eq_one}
    and \ref{lmm:toInteger_eval_cyclo}.
\end{proof}
...
\begin{proof}
    ...
    Let $S=(a,b,c,u)$. Let $a', b'$ be the units given by \ref{lmm:ex_dvd_a_add_b}.
...
\begin{proof}
    ...
    $\lambda \divides (a + b) + \lambda^2 \cdot b + 2 \cdot \lambda \cdot b
    = a + \eta^2 \cdot b$.
\end{proof}
...
\begin{proof}
    ...
    By way of contradiction using \ref{lmm:a_add_eta_b}, if $\lambda^2 \divides a +\eta\cdot b = a + b + \lambda \cdot b$,
    since $\lambda^2 \divides a+b$, then $\lambda^2 \divides \lambda \cdot b$, from which it follows
...
\begin{proof}
    ...
    By way of contradiction using \ref{lmm:toInteger_eval_cyclo},
    if $\lambda^2 \divides a +\eta^2\cdot b = a + b -b + \eta^2 \cdot b$,
    since $\lambda^2 \divides a+b$, then $\lambda^2 \divides b \cdot (\eta^2 -1)
    = \lambda \cdot b \cdot (\eta + 1)$. Since $\lambda \notdivides b$, then
...
\begin{proof}
    ...
    Trivial calculation using \ref{lmm:toInteger_eval_cyclo}.
\end{proof}
...
\begin{definition}[$x,y,z,w$]
    ...
    \uses{def:Solution}
    Let $S=(a, b, c, u)$ be a $solution$.\\\\
    We define $x \in \cc{O}_K$ such that $a + b = \lambda^{3n-2} \cdot x$.\\
...
\begin{proof}
    ...
    By way of contradiction, if $\lambda \divides y$, then $\lambda^2 \divides \lambda \cdot y
    = a + \eta\cdot b$ which is absurd by \ref{lmm:lambda_sq_not_dvd_a_add_eta_mul_b}.
\end{proof}
...
\begin{proof}
    ...
    By way of contradiction, if $\lambda \divides z$, then $\lambda^2 \divides \lambda \cdot z
    = a + \eta^2\cdot b$ which is absurd by \ref{lmm:lambda_sq_not_dvd_a_add_eta_sq_mul_b}.
\end{proof}
...
\begin{proof}
    ...
    lmm:lambda_ne_zero}
    By definition of multiplicity $\lambda^n \divides c$. Since $u$ is a unit, then using \ref{lmm:cube_add_cube_eq_mul}
    $\lambda^{3n} \divides u \cdot c^3 = a^3 + b^3
    = (a+b) \cdot (a + \eta \cdot b) \cdot (a + \eta^2 \cdot b)
    = (a+b)\cdot(\lambda\cdot y)\cdot(\lambda\cdot z)$.
    Then using \ref{lmm:lambda_not_dvd_y} and \ref{lmm:lambda_not_dvd_z},
...
\begin{proof}
    \leanok
    \uses{lmm:multiplicity_lambda_c_finite}
    By way of contradiction, if $\lambda \divides w$, then $\lambda^{n+1} \divides \lambda^n \cdot w
...
\begin{proof}
    ...
    $\lambda^{3n-1} \divides \lambda^{3n-2} \cdot x = a+b$. Using \ref{lmm:lambda_dvd_a_add_eta_mul_b}
    and \ref{lmm:lambda_dvd_a_add_eta_sq_mul_b}, we have that $\lambda^{3n+1} \divides
    (a+b) \cdot (a + \eta \cdot b) \cdot (a + \eta^2 cdot b) = a^3+b^3
    = u\cdot c^3 = u\cdot \lambda^{3n}\cdot w^3$.
    Then $\lambda \divides w^3$ which implies that $\lambda \divides w$
    which is absurd by \ref{lmm:lambda_not_dvd_w}.
\end{proof}
...
\begin{definition}[$u_1,u_2,u_3,u_4,u_5,X,Y,Z$]
    ...
    such that $z = u_3 \cdot Z^3$.\\
    We define $u_4 = \eta \cdot u_3 \cdot u_2^{-1}$.\\
    We define $u_5 = s-\eta^2 \cdot u_1 \cdot u_2^{-1}$.\\
\end{definition}
...
\begin{lemma}
    ...
    \uses{def:Solution}
    Let $S$ be a $solution$ with multiplicity $n$.\\\\
    Then $u_1 \cdot X^3 \cdot \lambda^{3n-2}+u_2 \cdot\eta\cdot Y^3\cdot \lambda +
    u_3\cdot \eta^2 \cdot Z^3\cdot \lambda = 0$.
\end{lemma}
\begin{proof}
    \leanok
...
\begin{lemma}
    ...
    \uses{def:Solution, def:Solution_u1_u2_u3_u4_u5_X_Y_Z}
    Let $S$ be a $solution$ with multiplicity $n$.\\\\
    Then $Y^3 + u_4 \cdot Z^3 = u_5 \cdot (\lambda^(n-1) \cdot X)^3$.
\end{lemma}
\begin{proof}
    \leanok
...
\begin{lemma}
    ...
    \uses{def:Solution}
    Let $S$ be a $solution$ with multiplicity $n$.\\\\
    Then $\lambda^2 \divides u_5 \cdot (\lambda^{n - 1} \cdot X)^3$.
\end{lemma}
\begin{proof}
    \leanok
    \uses{lmm:Solution_two_le_multiplicity}
...
\begin{definition}[Final Solution']
    ...
    lmm:X_ne_zero}
    Let $S = (a,b,c,u)$ be a $solution$ with multiplicity $n$.\\
    Let $S_f' = (Y,u_4\cdot Z, \lambda^{n-1}\cdot X, u_5)$.\\\\
