...
\begin{proof}
    ...
    \begin{align*}
        ...
        \text{(by \Cref{def:Solution_u1_u2_u3_u4_u5_X_Y_Z} and \Cref{def:Solution_x_y_z_w})}&= (a+b) + \eta (a+\eta b) + \eta^2 (a+\eta^2 b) \\
        &= a (1 + \eta + \eta^2) + b (1 + \eta^4 + \eta^2) \\
        \text{(by \Cref{lmm:toInteger_cube_eq_one})} &= (a+b)(1+\eta+\eta^2)\\
        \text{(by \Cref{lmm:toInteger_eval_cyclo})} &= (a+b)(0) = 0
    \end{align*}
\end{proof}
...
\begin{proof}
    ...
    Using \Cref{lmm:eta_isUnit}, \Cref{lambda_ne_zero}, it suffices to show that
    ...
    \Cref{lmm:Solution_two_le_multiplicity} and \Cref{formula1}.
\end{proof}
