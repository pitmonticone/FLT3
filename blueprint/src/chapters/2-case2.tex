...
\begin{proof}
    ...
    Then using \ref{lambda_not_dvd_y} and \ref{lambda_not_dvd_z},
...
\begin{proof}
    ...
    $\lambda^{3n-1} \divides \lambda^{3n-2} \cdot x = a+b$. Using \ref{lambda_dvd_a_add_eta_mul_b}
    and \ref{lambda_dvd_a_add_eta_sq_mul_b}, we have that $\lambda^{3n+1} \divides
    (a+b) \cdot (a + \eta \cdot b) \cdot (a + \eta^2 cdot b) = a^3+b^3
    = u\cdot c^3 = u\cdot \lambda^{3n}\cdot w^3$ and

    which is absurd by definition of multiplicity.
\end{proof}
